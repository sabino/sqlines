...
void SqlParser::BigQueryConvertRowLevelTrigger(Token *table, Token *when, Token *insert, Token *update, 
...
void SqlParser::BigQueryAppendSubqueryAlias(Token *append, int *appended_subquery_aliases)
{
	if(append == NULL)
		return;
...
void SqlParser::BigQueryDelimiter()
{
	if(Source(SQL_BIGQUERY, SQL_SYBASE) == false || Target(SQL_BIGQUERY, SQL_SYBASE) == true)
		return;
...
void SqlParser::BigQueryAddStmtDelimiter(bool force)
{
	if(Source(SQL_BIGQUERY, SQL_SYBASE) == false)
		return;
...
bool SqlParser::BigQueryGoDelimiter(bool just_remove)
{
	if(Source(SQL_BIGQUERY, SQL_SYBASE) == false || Target(SQL_BIGQUERY, SQL_SYBASE) == true)
		return false;
...
bool SqlParser::ParseBigQuerySetOptions(Token *set)
{
	if(set == NULL)
		return false;
...
bool SqlParser::ParseBigQueryIndexOptions(Token *token)
{
	if(token == NULL)
		return false;
	...
	ParseBigQueryStorageClause();

	return exists;
}

// Convert interval +/- expression to DATEADD function
void SqlParser::BigQueryToDateAdd(Token *op, Token *first, Token *first_end, Token *second, Token *second_end)
{
...
void SqlParser::ParseBigQueryExecProcedure(Token *execute, Token *name)
{
...
bool SqlParser::ParseBigQueryUpdateStatement(Token *update)
{
